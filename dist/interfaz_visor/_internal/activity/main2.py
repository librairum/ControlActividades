...
def generar_reporte_automatico(dni_usuario):
    try:
        log_path = os.path.join(os.getcwd(), 'reporte_log.txt')
        with open(log_path, 'a', encoding='utf-8') as log:
            ...
            log.write(f"\n[INICIO] Generando reporte a las {fin_local}\n")
            log.write(f"[DEBUG] SETTINGS_PATH: {SETTINGS_PATH}\n")
            if not os.path.exists(SETTINGS_PATH):
                log.write("[ERROR] No se encontró el archivo settings.json\n")
                return

            clases_json = cargar_reglas_desde_json(SETTINGS_PATH)
            reglas = compilar_reglas(clases_json)
            if not reglas:
                log.write("[ERROR] No se encontraron reglas de categorización.\n")
                return

            bucket = obtener_bucket_window()
            if not bucket:
                log.write("[ERROR] No se encontró el bucket aw-watcher-window_.\n")
                return
            ...
            if not eventos:
                log.write("[INFO] No se encontraron eventos en el rango especificado.\n")
                return
            ...
            if lista_a_insertar:
                log.write(f"[DEBUG] Se insertarán {len(lista_a_insertar)} actividades.\n")
                insertar_actividades(lista_a_insertar, CONFIG_PATH)
                log.write("[INFO] Actividades insertadas correctamente en la base de datos.\n")
            else:
                log.write("[INFO] No hubo actividades que cumplan los filtros para insertar.\n")

    except Exception as e:
        with open('reporte_log.txt', 'a', encoding='utf-8') as log:
            log.write(f"[ERROR] Error en reporte automático: {str(e)}\n")
